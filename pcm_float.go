...
func float32ToPCMInt32(value float32, bitDepth int) int32 {
	...
	switch bitDepth {
	case 16:
		sample := int64(math.Round(float64(value) * scalePCMInt16))
		if sample > maxPCMInt16 {
			sample = maxPCMInt16
		}
		...
		return int32(sample)
	case 24:
		sample := int64(math.Round(float64(value) * scalePCMInt24))
		if sample > maxPCMInt24 {
			sample = maxPCMInt24
		}
		...
		return int32(sample)
	case 32:
		sample := int64(math.Round(float64(value) * scalePCMInt32))
		if sample > maxPCMInt32 {
			sample = maxPCMInt32
		}
